...
t_move_choice=[18,32,46];%移动i个单位所需时间
...
%     CNC_state(n,:)=CNC_current;
    ...
    if(isempty(location_target))
        ...
        a=find(CNC_worktime>=545);
        if(~isempty(a))
        CNC_old=CNC_current;
        n=n+1;
        CNC_state(n,:)=CNC_current;
        CNC_current(find(CNC_worktime>=545))=0;
        CNC_worktime(find(CNC_worktime>=545))=0;
        end
%         CNC_old=CNC_current;
%         n=n+1;
%         CNC_state(n,:)=CNC_current;
    ...
    if(s>0)
        ...
        CNC_worktime(find(CNC_current==1))=CNC_worktime(find(CNC_current==1))+delta_t;
        a=find(CNC_worktime>=545);%如果重置时间
        if(~isempty(a))
        CNC_old=CNC_current;
        n=n+1;
        CNC_state(n,:)=CNC_current;
