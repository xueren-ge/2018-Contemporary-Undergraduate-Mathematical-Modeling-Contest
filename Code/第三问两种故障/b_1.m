...
while(iter<5000)
 %%
	...
	CNC_order_state_1=[0,0,0,0];%用于记录CNCi正处理哪个序号的物料
    CNC_order_state_2=[0,0,0,0];%用于记录CNCi正处理哪个序号的物料
    CNC_1=[0,0,0,0];%0表示CNC未在加工,第一道工序CNC
    CNC_2=[0,0,0,0];%0表示CNC未在加工，第二道工序CNC
    CNC_worktime_1=[0,0,0,0];%第一道工序CNC已加工时间
    CNC_worktime_2=[0,0,0,0];
    CNC_failuretime_1=zeros(1,4);%故障计时
    CNC_failuretime_2=zeros(1,4);%故障计时
    CNC_repairtime_1=zeros(1,4);%下一次故障时修复好的时间
    CNC_repairtime_2=zeros(1,4);%下一次故障时修复好的时间
    t_delta=0;
    time_all=0;
